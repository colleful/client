...
const SentMatchingListScreen = ({teamId}) => {
  ...
  const {data:sentMatchingList = [], error} = useSWR(teamId === null ? null :`${Config.baseUrl}/api/matching/sent`, fetcher, {
  ...
  return (
    <View style={css`flex: 1; padding-top: 100px; padding-horizontal: 25px`}>
      ...
      <View style={css`border-bottom-width: 1px; margin-bottom: 20px`} />
        {( sentMatchingList != null && typeof sentMatchingList == "object" && !Object.keys(sentMatchingList).length )
